...
export function ThemeCustomizationProvider(props: {
  ...
  useEffect(() => {
    // Always write the preset attribute. The CSS baseline (:root in theme.css)
    // is the 'default' preset, and theme-presets.css explicitly handles an
    // attribute value of 'default' via :not([data-theme-preset='default'])
    // selectors. Dropping the attribute only when it equals the app default
    // would break a non-'default' default preference (e.g. 'anthropic'):
    // removing it falls back to the blue :root baseline instead of applying
    // the intended preset.
