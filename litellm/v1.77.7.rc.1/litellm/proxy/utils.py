...
async def count_tokens_with_anthropic_api(
    ...
    try:
        ...
        if deployment is not None:
            ...
            anthropic_api_key = deployment_litellm_params.get("api_key")
            litellm_credential_name = deployment_litellm_params.get("litellm_credential_name")

            if not anthropic_api_key and litellm_credential_name is not None:
                from litellm.litellm_core_utils.credential_accessor import CredentialAccessor
                anthropic_credential = CredentialAccessor.get_credential_values(litellm_credential_name)
                if anthropic_credential is not None:
                    anthropic_api_key = anthropic_credential.get("api_key")

        # Fallback to environment variable
        if not anthropic_api_key:
            anthropic_api_key = os.getenv("ANTHROPIC_TOKEN_COUNT_API_KEY")
